...
%DIF DEL revision1/NSE19-11RevisedClean.tex    Thu Apr 11 12:41:08 2019
%DIF ADD revision2/NSE19-11RevisedClean2.tex   Fri May  3 17:12:58 2019
...
\alpha_1|^{-1}$, then the fundamental mode might not contribute significanlty 
...
To the authors' knowledge the application of DMD to neutronics problems is limited \cite{abdo2018data} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{mcclarren2018calculating}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{mcclarren2019calculating}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .  
...
However, the dynamic modes can only contain what the snapshots contain, so if 
the snapshots lack a thermal neutron component, so will the dynamic modes.  
This suggests that fewer dynamic modes should be required for our problems than 
...
For instance, 
\DIFdelbegin %DIFDELCMD < \bea
%DIFDELCMD < 	%%%
\DIFdel{\vec{v}(\Delta t) }%DIFDELCMD < &%%%
\DIFdel{=}%DIFDELCMD < & %%%
\DIFdel{\sum_{i=1}^{N-1} a_i \vec{z}_i \exp{(\omega_i \Delta t)} \pec }%DIFDELCMD < \nonumber \\
%DIFDELCMD < 	&%%%
\DIFdel{=}%DIFDELCMD < & %%%
\DIFdel{\sum_{i=1}^{N-1} a_i \vec{z}_i \lambda_i \pec }%DIFDELCMD < \nonumber \\ 
%DIFDELCMD < 	&%%%
\DIFdel{=}%DIFDELCMD < & %%%
\DIFdel{\mA \vec{v}_1 \pec }%DIFDELCMD < \nonumber \\ 
%DIFDELCMD < 	&%%%
\DIFdel{=}%DIFDELCMD < & %%%
\DIFdel{\vec{v}_2 \pep
	}\DIFdelend \DIFaddbegin \be
\begin{aligned}
	\vec{v}(\Delta t) &= \sum_{i=1}^{N-1} a_i \vec{z}_i \exp{(\omega_i \Delta t)}  \\
	&= \sum_{i=1}^{N-1} a_i \vec{z}_i \lambda_i  \\ 
	&= \mA \vec{v}_1  \\ 
	&= \vec{v}_2 \pep
\end{aligned}
	\DIFaddend \LEQ{2.5d}
\DIFdelbegin %DIFDELCMD < \eea 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \ee
\DIFaddend Note that any eigenfunction with a zero-eigenvalue will have no dynamic content because it will instantly attenuate to zero.  
...
\bt[h] \centering 
	...
	\btb{|c|c|c|c|}
		...
		$\chi$  & 1 & 0 & 0 \\ 	\hline
		\DIFdelbegin \DIFdel{$\nu\sigma_f \ [b]$ }\DIFdelend \DIFaddbegin \DIFadd{$\nu\sigma_f \ (b)$ }\DIFaddend & 5.4 & 60.8 & 28.0 \\  \hline
		\DIFdelbegin \DIFdel{$\sigma_R \ [b]$  }\DIFdelend \DIFaddbegin \DIFadd{$\sigma_R \ (b)$  }\DIFaddend & 3.31 & 36.2 & 13.6 \\  \hline
		\DIFdelbegin \DIFdel{$\sigma_t \ [b]$ }\DIFdelend \DIFaddbegin \DIFadd{$\sigma_t \ (b)$ }\DIFaddend & 7.71 & 50.0 & 25.6\\ \hline
		\DIFdelbegin \DIFdel{$v \ [cm/\mu s]$ }\DIFdelend \DIFaddbegin \DIFadd{$v \ (cm/\mu s)$ }\DIFaddend & 2000 & 100 & 2.2 \\  \hline
	\etb 
	 \LTA{xs_data}
\et
...
\ee
where \DIFaddbegin \newline
\indent \DIFaddend $n$ \DIFdelbegin \DIFdel{gives the }\DIFdelend \DIFaddbegin \DIFadd{= }\DIFaddend index of the spatial mode \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \newline
\indent \DIFaddend $m$ \DIFdelbegin \DIFdel{gives the }\DIFdelend \DIFaddbegin \DIFadd{=  }\DIFaddend index of the energy modes corresponding to each spatial mode \DIFdelbegin \DIFdel{.  
}\DIFdelend \DIFaddbegin \newline
\indent \DIFaddend $A_{nm}$ \DIFdelbegin \DIFdel{is the coeffient for the }\DIFdelend \DIFaddbegin \DIFadd{= }\DIFaddend $n,m$'th space-energy mode \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{amplitude }\newline
\indent \DIFaddend $\varphi_n(r)$ \DIFdelbegin \DIFdel{is 
the }\DIFdelend \DIFaddbegin \DIFadd{= }\DIFaddend $n$'th spatial mode \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \newline
\indent \DIFaddend $f_{nm, g}$ \DIFdelbegin \DIFdel{is the }\DIFdelend \DIFaddbegin \DIFadd{= }\DIFaddend $g$'th component of the $n,m$'th space-energy mode \DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \newline 
\indent \DIFaddend $\alpha_{nm}$ \DIFdelbegin \DIFdel{is the }\DIFdelend \DIFaddbegin \DIFadd{= }\DIFaddend time eigenvalue for the $n,m$'th space-energy mode. \DIFaddbegin \newline

\DIFaddend The spatial modes are those of the Laplace equation for a spherical geometry given by \EQ{eigfunc}, with the same boundary conditions as those in the governing equation.  
\be
...
Because the eigenproblem\DIFdelbegin \DIFdel{described above }\DIFdelend \DIFaddbegin \DIFadd{, given in \EQ{eigprob}, }\DIFaddend is not symmetric, the adjoint (left) eigenfunctions given by, $f_{nm}^* \varphi_n(r)$, must be used.  
...
Consider \EQ{cell_eqn}, the conservation equation for the i$^{\text{th}}$ cell\DIFdelbegin \DIFdel{,
...
The reader should note that the source term is simply the fission and 
scattering source.  
...
\ee
where \DIFdelbegin \DIFdel{$V_i $ is the }\DIFdelend \DIFaddbegin \newline
\indent \DIFadd{$V_i$ = }\DIFaddend volume of cell $i$ \DIFdelbegin \DIFdel{, $ A_{i+1/2} $ is the }\DIFdelend \DIFaddbegin \newline
\indent \DIFadd{$A_{i+1/2}$ = }\DIFaddend surface area of the outer edge of cell $i$ \DIFdelbegin \DIFdel{, and $ J_{i+1/2} = D_g \nabla \phi_{g,i} $ is the }\DIFdelend \DIFaddbegin \newline
\indent \DIFadd{$J_{i+1/2}$ = $D_g \grad \phi_{g,i}$ = }\DIFaddend current at outer edge of cell $i$. \DIFaddbegin \newline

\DIFaddend Central difference is now applied to the \DIFaddbegin \DIFadd{neutron }\DIFaddend current terms with respect to adjacent cell-centers to obtain the second-order accurate scheme, as shown in \EQ{space_discretized}.  
\be
	\frac{V_i}{v_g} \dxdt{\phi_{i,g}} - A_{i+\half} \DIFdelbegin \DIFdel{\frac{\phi_{i+1,g} - \phi_{i,g}}{h} }\DIFdelend \DIFaddbegin \DIFadd{\frac{D_g}{h} }\fn{\phi_{i+1,g} - \phi_{i,g}} \DIFaddend + A_{i-\half} \DIFdelbegin \DIFdel{\frac{\phi_{i,g} 
		- \phi_{i-1,g}}{h} }\DIFdelend \DIFaddbegin \DIFadd{\frac{D_{g}}{h} }\fn{\phi_{i,g} - \phi_{i-1,g}} \DIFaddend + V_i \Sigma_{R,g} \phi_{i,g} = V_i S_{i,g},
...
\be
	\frac{2 \fn{ f^{n+\half} - f^n }}{\Delta t} =  \half \mA \fn{f^{n+\half} + 
	f^n}.
	\LEQ{halfCN} 
\ee
The result of solving for $f^{n+\half}$ is then plugged into second-order 
backwards difference to obtain the solution at the end of the time-step, shown 
in \EQ{halfBDF}.  
\be
	\frac{3 \fn{ f^{n+1} - f^{n+\half} }}{\Delta t} - \frac{f^{n+\half} - 
		f^n}{\Delta t} = \mA f^{n+1}.
	\LEQ{halfBDF} 
\ee 
Implementing this is relatively straightforward. 
The time derivative term containing the scalar flux at the next half time-step 
...
In addition, the performance of DMD on the scalar group flux and the physical 
insights gained will be compared with that of an $\alpha$-eigenfunction 
computation.  
...
The relative singular value spectrum, where $\sigma$'s are the singular values, 
ordered $\sigma_0 > \sigma_1 > ... > \sigma_{N-1}$, of $\mX_1^{N-1}$ is shown 
in \FI{sv-spec}.  
...
If the snapshot series is denoted as $\mX$, and the DMD representation of 
$\mX$, $\mX_{DMD}$, theory states that the error in the Frobenius norm should 
be proportional to the magnitude of the largest truncated singular value, given 
by $\lVert \mX - \mX_{DMD} \rVert_F \sim \sigma_{K+1}$ \cite{kutz2016dynamic}.  
The code verification is demonstrated in \FI{recovery}.  
\bfg[h] \centering
	$
	\begin{array}{cc}
	...
	\LFI{recovery}
\efg
Practically, only the number of modes corresponding to error on the order of 
...
The $\alpha$ modes are denoted as such, and the dynamic modes denoted with 
$\omega$.  
...
For this reason, it is expected that the dynamic modes bear some resemblence to 
them in shape and time-scale.  
...
Because no thermal modes contribute to the system, the epithermal modes set the 
...
It not only shows greater efficiency than an $\alpha$-eigenvalue computation, 
but also preserves the physical insights which can be gained.  
Further, the DMD solution produces accurate results both at snapshot sampling 
times and in between them, as well as far after the sampling period.  

\subsection{DMD with the Fission Rate}
For real applications, the size of the solution vector may become restrictively 
large.  
...
against each other at 3 distinctive times, 1/3, 1/2, and 2/3 of the simulation 
time, to depict the solution at times both on and in between times 
...
The reason for this is believed to be because the fission rate does not have a 
solution of the same form as the DMD solution, and therefore, as was the case 
before, the long-term behaviors cannot be sufficiently captured.  
The reader should take note that the error only increased by 2 orders of 
magnitude after 4 times the sampling period.  
While extrapolation errors are likely to be problem dependent, this shows that 
DMD is likely able to extrapolate in time to predict nearby future states, as 
suggested by Kutz \cite{kutz2016dynamic}.
...
rate.  
This method was then compared to a truncated $\alpha$-eigenfunction expansion.  
It was shown not only that DMD is both applicable for these problems, but that 
it has superior efficiency to $\alpha$-eigenfunction expansions while retaining the same level of physical insight.  
The $\alpha$-eigenfunction expansion is based purely on physics, not the 
specific problem, and thus, has no means of filtering out thermal modes. 
This results in many wasted computations on modes which do not contribute to 
the solution.  
In contrast, DMD is data driven, which by definition, filters out unimportant 
portions of phase space and keeps only relevant dynamic structures.  
The DMD solution maintained accuracy when interpolating between snapshots for 
both the group flux and fission rate, and predicted nearby future states 
...
In the future, this method will be expanded to nonlinear and multiphysics 
problems to more accurately reflect problems in this space.  
This method has been widely adapted to nonlinear systems in the turbulent fluid 
flow community \cite{kutz2016dynamic} \cite{schmid2010dynamic}, but to the 
authors' knowledge, its application to multiphysics problems is limited.
Lastly, the different flavors of the DMD algorithm will be explored to find 
the algorithm best tailored for this these problems.

\section*{Acknowledgements}
This work was supported by the U.S. Department of Energy through the Los Alamos 
...
Graduate Research Fellowship under Grant No. DGE:1746932.  
Any opinion, findings, and conclusions or recommendations expressed in this 
material are those of the authors(s) and do not necessarily reflect the views 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{bibliography_RevisedNew}
\DIFaddend 
