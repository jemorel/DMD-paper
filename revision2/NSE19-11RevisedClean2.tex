...
To the authors' knowledge the application of DMD to neutronics problems is limited \cite{abdo2018data} \cite{mcclarren2019calculating}.  
...
\ee
where \newline
\indent $n$ = index of the spatial mode \newline
\indent $m$ =  index of the energy modes corresponding to each spatial mode \newline
\indent $A_{nm}$ = $n,m$'th space-energy mode amplitude \newline
\indent $\varphi_n(r)$ = $n$'th spatial mode \newline
\indent $f_{nm, g}$ = $g$'th component of the $n,m$'th space-energy mode \newline 
\indent $\alpha_{nm}$ = time eigenvalue for the $n,m$'th space-energy mode. \newline
...
Because the eigenproblem, given in \EQ{eigprob}, is not symmetric, the adjoint (left) eigenfunctions given by, $f_{nm}^* \varphi_n(r)$, must be used.  
...
\ee
where \newline
\indent $V_i$ = volume of cell $i$ \newline
\indent $A_{i+1/2}$ = surface area of the outer edge of cell $i$ \newline
\indent $J_{i+1/2}$ = $D_g \grad \phi_{g,i}$ = current at outer edge of cell $i$. \newline

Central difference is now applied to the neutron current terms with respect to adjacent cell-centers to obtain the second-order accurate scheme, as shown in \EQ{space_discretized}.  
\be
	\frac{V_i}{v_g} \dxdt{\phi_{i,g}} - A_{i+\half} \frac{D_g}{h} \fn{\phi_{i+1,g} - \phi_{i,g}} + A_{i-\half} \frac{D_{g}}{h} \fn{\phi_{i,g} - \phi_{i-1,g}} + V_i \Sigma_{R,g} \phi_{i,g} = V_i S_{i,g},
...
\bibliography{bibliography_RevisedNew}
