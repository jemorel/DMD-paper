...
In practice,  $\alpha$-eigenvalue calculations for this application are quite 
	computationally expensive, and  further, in most cases, analytic solutions are 
	not available. 
...
The singular value spectrum of  $\bm{X}^{N-1}_1$ is presented below in 
	\FI{sv-spec}, and shows that this condition is met.
...
When implementing the DMD method, one often may want to use fewer modes than the $N-1$, 
...
These results suggest that the DMD solution captured the dynamics of the longer
	lived modes such that as the system settles into those modes, DMD does as 
	well.
...
Because no thermals exist in the system, the computed amplitudes of the 
	thermal modes are zero, however, they must still be computed before any 
...
\bfg[!htb] \centering
	$
	\begin{array}{c c}
	\includegraphics[scale=0.5]{alpha0-3.png}
	\includegraphics[scale=0.5]{dmd0.png} \\
	\includegraphics[scale=0.5]{alpha1-3.png}
	\includegraphics[scale=0.5]{dmd1.png} \\
	\end{array}
	$
	\includegraphics[scale=0.5]{alpha2-3.png}
...
For this reason, it is expected that the DMD modes bear so resemblence to them in shape and 
time-scale.
To show this, the eigenvalues and eigenfunctions of the first 3 epithermal 
	...
	in \FI{modes}.
Because no thermal modes contribute to the system, the epithermal modes, in
	fact, set the decay rates for this problem.
...
In contrast to the previous comparison, the fundamental epithermal mode and the 
	first dynamic mode are visibly indistinguishable, and have identical decay 
	constants.
The shapes of the second mode appear to be nearly identical, and the DMD 
...
Because only a finite number of dynamic modes represent the system, it is not 
	...
	modes achieve the same level of accuracy as a much larger number of the 
	$\alpha$-eigenmodes.
...
For this reason, it would be  advantageous to apply DMD on some other lower dimensionsal quantity 
