...
%DIF DEL NSE19-11OldClean.tex       Sat Apr  6 16:41:45 2019
%DIF ADD NSE19-11RevisedClean.tex   Sat Apr  6 16:58:30 2019
...
In addition, the performance of DMD on the scalar group flux \DIFdelbegin \DIFdel{will }\DIFdelend and the physical 
...
The \DIFaddbegin \DIFadd{relative }\DIFaddend singular value spectrum\DIFdelbegin \DIFdel{of  $\bm{X}^{N-1}_1$ is presented below in \FI{sv-spec}, and shows that this condition is met}\DIFdelend \DIFaddbegin \DIFadd{, where $\sigma$'s are the singular values, 
ordered $\sigma_0 > \sigma_1 > ... > \sigma_{N-1}$, of $\mX_1^{N-1}$ is shown 
in \FI{sv-spec}}\DIFaddend .  
...
If the snapshot series is denoted as \DIFdelbegin \DIFdel{$X$}\DIFdelend \DIFaddbegin \DIFadd{$\mX$}\DIFaddend , and the DMD representation of 
\DIFdelbegin \DIFdel{$X$, $X_{DMD}$}\DIFdelend \DIFaddbegin \DIFadd{$\mX$, $\mX_{DMD}$}\DIFaddend , theory states that the error in the Frobenius norm should 
be proportional to the magnitude of the largest truncated singular value, given 
by \DIFdelbegin \DIFdel{$\lVert X - X_{DMD} \rVert_F \sim \sigma_{K+1}$ \mbox{%DIFAUXCMD
\cite{kutz2016dynamic}}%DIFAUXCMD
.  
This is }\DIFdelend \DIFaddbegin \DIFadd{$\lVert \mX - \mX_{DMD} \rVert_F \sim \sigma_{K+1}$ \mbox{%DIFAUXCMD
\cite{kutz2016dynamic}}%DIFAUXCMD
.  
The }\DIFaddend code verification is demonstrated in \FI{recovery}.  
\bfg[h] \centering
	$
	\begin{array}{cc}
		\includegraphics[scale=0.5]{figures/flux_cutoffStudy.jpg} &
		\includegraphics[scale=0.5]{figures/flux_rankStudy.jpg}
	\end{array}
	$
...
\bfg[t] \centering
	$
	\begin{array}{cc}
		\includegraphics[scale=0.45]{figures/flux_t3ns.jpg} &
		\includegraphics[scale=0.45]{figures/flux_t5ns.jpg} \\
	\end{array}
	$
	\includegraphics[scale=0.45]{figures/flux_t6ns.jpg}
...
The $\alpha$ modes are denoted as such, and the \DIFdelbegin \DIFdel{DMD }\DIFdelend \DIFaddbegin \DIFadd{dynamic }\DIFaddend modes denoted with 
$\omega$.  
Each mode is normalized to its $L^2$ norm, so that an accurate comparison can be made.  
\bfg[!htb] \centering
	$
	\begin{array}{c c}
		\includegraphics[scale=0.5]{figures/alpha1-3.jpg} &
		...
		\includegraphics[scale=0.5]{figures/alpha3-3.jpg} &
		\includegraphics[scale=0.5]{figures/dmd3.jpg}
	\end{array}
	$
...
For this reason, it is expected that the \DIFdelbegin \DIFdel{DMD modes bear so }\DIFdelend \DIFaddbegin \DIFadd{dynamic modes bear some }\DIFaddend resemblence to 
...
Because no thermal modes contribute to the system, the epithermal modes \DIFdelbegin \DIFdel{, set the 
decay rates }\DIFdelend \DIFaddbegin \DIFadd{set the 
time scale }\DIFaddend for this problem.  
\bfg[!htb] \centering
	$
	\begin{array}{c c}
		\includegraphics[scale=0.5]{figures/alpha1-2.jpg} &
		\includegraphics[scale=0.5]{figures/dmd1.jpg} \\
...
It not only \DIFdelbegin \DIFdel{showed }\DIFdelend \DIFaddbegin \DIFadd{shows }\DIFaddend greater efficiency than an $\alpha$-eigenvalue computation, 
but also \DIFdelbegin \DIFdel{preserved }\DIFdelend \DIFaddbegin \DIFadd{preserves }\DIFaddend the physical insights which can be gained.  
Further, the DMD solution \DIFdelbegin \DIFdel{produced }\DIFdelend \DIFaddbegin \DIFadd{produces }\DIFaddend accurate results both at snapshot sampling 
...
\bfg[!htb] \centering
	\includegraphics[scale=0.5]{figures/fis_singularValueSpectrum.jpg}
...
\DIFdelbegin \DIFdel{, }\DIFdelend indicates that a low-rank structure exists within the space-dependent fission 
...
\bfg[!htb] \centering
	$
	\begin{array}{c c}
		\includegraphics[scale=0.5]{figures/fis_cutoffStudy.jpg} &
		\includegraphics[scale=0.5]{figures/fis_rankStudy.jpg}
	\end{array}
	$
...
time, to depict the solution at times both on \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend in between times 
corresponding to the snapshots used in DMD.  
This is shown in \FI{fis-ex}.
\bfg[!htb] \centering
	$
	\begin{array}{c c}
		\includegraphics[scale=0.5]{figures/fis_t3ns.jpg} &
		\includegraphics[scale=0.5]{figures/fis_t5ns.jpg}
	\end{array}
	$
	\includegraphics[scale=0.5]{figures/fis_t6ns.jpg}
...
The reader should take note that the \DIFdelbegin \DIFdel{increase in }\DIFdelend error only increased by 2 orders of 
magnitude after 4 times the sampling period.  
\DIFaddbegin \DIFadd{While extrapolation errors are likely to be problem dependent, this shows that 
DMD is likely able to extrapolate in time to predict nearby future states, as 
suggested by Kutz \mbox{%DIFAUXCMD
\cite{kutz2016dynamic}}%DIFAUXCMD
.
}\DIFaddend 
...
rate\DIFdelbegin \DIFdel{and to compare it to }\DIFdelend \DIFaddbegin \DIFadd{.  
This method was then compared to }\DIFaddend a truncated $\alpha$-eigenfunction expansion.  
It was shown \DIFdelbegin \DIFdel{that not only can DMD be applied to these problemssuccessfully}\DIFdelend \DIFaddbegin \DIFadd{not only that DMD is both applicable for these problems}\DIFaddend , but that 
\DIFdelbegin \DIFdel{DMD shows far superior efficiency to other methods }\DIFdelend \DIFaddbegin \DIFadd{it has superior efficiency }\DIFaddend while retaining the same level of physical insight.  
\DIFdelbegin \DIFdel{Unlike an }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend $\alpha$-eigenfunction expansion \DIFdelbegin \DIFdel{, where many thermal modeswith zero-amplitudes must be computed before contibuting modes , dynamic modes, because they are data driven, are guaranteed to 
be relevant to }\DIFdelend \DIFaddbegin \DIFadd{is based purely on physics, not the 
...
}\DIFaddend the solution.  
In \DIFdelbegin \DIFdel{fact, no thermal neutrons are present in any dynamic modes.  
The continuous DMD solution did not lose }\DIFdelend \DIFaddbegin \DIFadd{contrast, DMD is data driven, which by definition, filters out unimportant 
portions of phase space and keeps only relevant dynamic structures.  
The DMD solution maintained }\DIFaddend accuracy when interpolating between snapshots \DIFdelbegin \DIFdel{, and proved to maintain accuracybeyong the last sampling time}\DIFdelend \DIFaddbegin \DIFadd{for 
both the group flux and fission rate, and predicted nearby future states 
without significant losses in accuracy}\DIFaddend .  
...
In the future, this method will be expanded to \DIFdelbegin \DIFdel{non-linear }\DIFdelend \DIFaddbegin \DIFadd{nonlinear }\DIFaddend and multiphysics 
problems \DIFdelbegin \DIFdel{, }\DIFdelend to more accurately reflect \DIFdelbegin \DIFdel{the problems of interest.  
The prior is anticipated to have success because of the robust application of DMD to non-linear }\DIFdelend \DIFaddbegin \DIFadd{problems in this space.  
This method has been widely adapted to nonlinear }\DIFaddend systems in the turbulent fluid 
flow community \cite{kutz2016dynamic} \cite{schmid2010dynamic}\DIFaddbegin \DIFadd{, but to the 
authors' knowledge, its application to multiphysics problems is limited}\DIFaddend .
Lastly, \DIFdelbegin \DIFdel{an exploration of }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend different flavors of the DMD algorithm \DIFdelbegin \DIFdel{should be explored in order }\DIFdelend \DIFaddbegin \DIFadd{will be explored }\DIFaddend to find 
the \DIFdelbegin \DIFdel{best tailored algorithm for problems within this application space would be a useful endeavor}\DIFdelend \DIFaddbegin \DIFadd{algorithm best tailored for this these problems}\DIFaddend .

\section*{Acknowledgements}
This \DIFdelbegin \DIFdel{information has been authored by employees of }\DIFdelend \DIFaddbegin \DIFadd{work was supported by the U.S. Department of Energy through }\DIFaddend the Los Alamos 
National \DIFdelbegin \DIFdel{Security, LLC (LANS) operator of }\DIFdelend \DIFaddbegin \DIFadd{Laboratory. }\DIFaddend Los Alamos National Laboratory \DIFdelbegin \DIFdel{under contract 
No DE-AC52-06NA25396 with the }\DIFdelend \DIFaddbegin \DIFadd{is operated by Triad 
National Security, LLC, for the National Nuclear Security Administration of 
}\DIFaddend U.S. Department of Energy \DIFaddbegin \DIFadd{(Contract No. 89233218CNA000001).
}

\DIFadd{This material is based upon work supported by the National Science Foundation 
Graduate Research Fellowship under Grant No. (NSF grant number).  
...
of the National Science Foundation}\DIFaddend .  
